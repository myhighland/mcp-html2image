...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === 'capture-html-to-image') {
            const { html, outputPath, format = 'png', width = 1200, height = 800, quality = 90, fullPage = false, selector, omitBackground = false, } = args;
        ...
        if (name === 'capture-url-to-image') {
            const { url, outputPath, format = 'png', width = 1200, height = 800, quality = 90, fullPage = false, selector, omitBackground = false, waitUntil = 'networkidle2', timeout = 30000, } = args;
            const output = outputPath || `capture_${Date.now()}.${format}`;
